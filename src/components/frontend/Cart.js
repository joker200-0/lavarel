...
function Cart()
{
    ...
    //useEffect cho phép chúng ta return 1 function
    //function này sẽ thực thi trước khi mà component đó được ngắt kết nối.
    useEffect(() => {
    ...
    const handleDecrement = (cart_id) => {
        setCart(cart => 
            cart.map( (item) => 
            //item qty -(kiểm tra item.product_qty > 1 trả về 1 ngược lại trả về 0)  
    ...
    //cập nhật sản phẩm muốn mua trong giỏ hàng 
    ...
    const deleteCartItem = (e, cart_id) => {
        e.preventDefault();
        //currentTarget
        //Lấy phần tử kích hoạt sự kiện:
      
        const thisClicked = e.currentTarget;
        //sau đó xóa đi  chữ trong thẻ 
        ...
        axios.delete(`/api/delete-cartitem/${cart_id}`).then( res => {
            ...
            {
                swal("Success",res.data.message,"success");
                //xóa đi cái thẻ tr in ra cái đòng đó 
